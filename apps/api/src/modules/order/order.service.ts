...
export class OrderService {
  ...
  /**
   * Generic platform customer names that should not overwrite real names
   */
  ...
  private isGenericCustomerName(name: string | null | undefined): boolean {
    ...
    return OrderService.GENERIC_CUSTOMER_NAMES.some(
  ...
  private async findOrCreateCustomer(
    ...
    if (!customer) {
      ...
      // - If existing customer has a real name, keep it unless new name is also real
      // - If existing customer has a generic name and new name is real, use new name
      // - Never overwrite a real name with a generic platform name
      const existingNameIsGeneric = this.isGenericCustomerName(customer.name);
      const newNameIsGeneric = this.isGenericCustomerName(customerData.name);

      let nameToUse = customer.name;
      if (existingNameIsGeneric && !newNameIsGeneric) {
        // Upgrade from generic to real name
        nameToUse = customerData.name;
        this.logger.log(
          `Upgrading customer ${customer.id} name from "${customer.name}" to "${customerData.name}"`,
        );
      } else if (!existingNameIsGeneric && newNameIsGeneric) {
        // Keep existing real name, don't overwrite with generic
        this.logger.debug(
          `Keeping existing name "${customer.name}" for customer ${customer.id}, ignoring generic "${customerData.name}"`,
        );
      } else if (!existingNameIsGeneric && !newNameIsGeneric && customerData.name) {
        // Both are real names, use the new one (it might be more current)
        nameToUse = customerData.name;
      }
