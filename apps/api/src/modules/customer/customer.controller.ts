...
export class CustomerController {
  ...
  /**
   * POST /customers/fix-generic-names
   * Fix generic customer names by looking at order history
   */
  @Post('fix-generic-names')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Fix generic customer names',
    description: 'Fix customers with generic names (like "Trendyol Müşteri") by looking at their order history for real names',
  })
  @ApiResponse({
    status: 200,
    description: 'Generic names fixed successfully',
    schema: {
      type: 'object',
      properties: {
        fixed: { type: 'number', description: 'Number of customers fixed' },
        total: { type: 'number', description: 'Total customers with generic names' },
      },
    },
  })
  @Roles('ADMIN')
  fixGenericNames() {
    return this.customerService.fixGenericCustomerNames();
  }
}
