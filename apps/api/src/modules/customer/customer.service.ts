...
export class CustomerService {
  ...
  /**
   * Generic platform customer names that should not be considered real names
   */
  private static readonly GENERIC_CUSTOMER_NAMES = [
    'trendyol müşteri',
    'trendyol müşterisi',
    'trendyol musteri',
    'trendyol customer',
    ...
    'migros customer',
    'yemeksepeti müşteri',
    'yemeksepeti müşterisi',
    'yemeksepeti musteri',
    'yemeksepeti customer',
    'platform müşteri',
    'müşteri',
    'musteri',
    'customer',
  ...
  /**
   * Check if a customer name is a generic platform name
   */
  private isGenericCustomerName(name: string | null | undefined): boolean {
    if (!name) return true;
    const normalized = name.toLowerCase().trim();
    return CustomerService.GENERIC_CUSTOMER_NAMES.some(
      (generic) => normalized === generic || normalized.includes(generic),
    );
  }

  /**
   * Fix generic customer names by looking at their order history
   * This is a one-time fix for customers that were incorrectly saved with generic names
   */
  async fixGenericCustomerNames(): Promise<{ fixed: number; total: number }> {
    this.logger.log('Starting generic customer names fix...');

    // Find all customers with generic names
    ...
    let fixed = 0;
    const customersWithGenericNames = customers.filter((c) =>
      this.isGenericCustomerName(c.name),
    );

    this.logger.log(
      `Found ${customersWithGenericNames.length} customers with generic names out of ${customers.length} total`,
    );

    for (const customer of customersWithGenericNames) {
      // Look for real name in order history
      const ordersWithNames = await this.prisma.order.findMany({
        where: {
          customerId: customer.id,
          customerName: { not: '' },
        },
      ...
      // Find the first non-generic name from orders
      let realName: string | null = null;
      for (const order of ordersWithNames) {
        if (order.customerName && !this.isGenericCustomerName(order.customerName)) {
      ...
      if (realName) {
        ...
        this.logger.log(
          `Fixed customer ${customer.id}: "${customer.name}" → "${realName}"`,
        );
        fixed++;
      }
    }
    ...
    this.logger.log(
      `Generic customer names fix completed: ${fixed} fixed out of ${customersWithGenericNames.length} generic names`,
    );

    return { fixed, total: customersWithGenericNames.length };
  }
}
