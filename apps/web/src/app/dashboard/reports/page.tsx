...
const mockReportData = {
  ...
  // Restaurant hours: 11:00 AM - 01:00 AM (next day)
  hourlyOrders: [
    { hour: '11:00', orders: 8 },
...
export default function ReportsPage() {
  ...
  const { data: reportData, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // API interceptor unwraps response, so response.data contains the actual data
        // Validate the response has the expected structure
        const data = response?.data || response;
        if (data?.summary?.totalRevenue !== undefined) {
          return data;
        }
        // If response doesn't have expected structure, use mock data
  ...
  // Ensure data always has the expected structure
  const data = reportData?.summary ? reportData : mockReportData;
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex border rounded-md">
            {(['today', 'this_week', 'this_month'] as const).map((range) => (
              ...
              >
                {range === 'today' ? 'Bugun' : range === 'this_week' ? 'Hafta' : 'Ay'}
              </Button>
            ))}
          </div>
          ...
          <Button
            variant="outline"
            onClick={() => {
              const reportContent = `312 Doner Rapor - ${dateRange === 'today' ? 'Bugun' : dateRange === 'this_week' ? 'Bu Hafta' : 'Bu Ay'}
