...
class TurnstileAPIServer:
    ...
    def _setup_routes(self) -> None:
        ...
        self.app.route('/migros-login', methods=['POST'])(self.migros_login)
    ...
    async def migros_login(self):
        """Perform Migros login with Turnstile solving - returns JWT tokens"""
        try:
            ...
            logger.info(f"Starting Migros login for {email}")
            ...
            # Intercept API responses to capture login result
            async def handle_response(response):
                nonlocal login_response
                if '/rest/User/Login' in response.url and response.status == 200:
                    try:
                        login_response = await response.json()
                        logger.debug(f"Captured login response")
                    except:
                        pass
            ...
            try:
                # Navigate to Migros login page
                logger.debug(f"Browser {index}: Navigating to Migros login page")
                await page.goto('https://restoran.migrosonline.com/login', wait_until='networkidle', timeout=30000)

                # Wait for login form
                await page.wait_for_selector('input[type="email"], input[name="email"], input[placeholder*="mail"]', timeout=10000)
                logger.debug(f"Browser {index}: Login form loaded")
                ...
                logger.debug(f"Browser {index}: Credentials filled, waiting for Turnstile auto-solve")

                # Camoufox should auto-solve Turnstile - just wait for it
                # Look for the green checkmark or wait for iframe to complete
                await asyncio.sleep(3)  # Give initial time for Turnstile to load

                # Try to detect and interact with Turnstile
                for attempt in range(15):
                    try:
                        # Try to find and click the Turnstile checkbox/widget
                        # Different selectors for different Turnstile implementations
                        selectors = [
                        ...
                        for selector in selectors:
                            ...
                            if element:
                                ...
                                if box:
                                    ...
                                    logger.debug(f"Browser {index}: Clicked Turnstile widget")
                    ...
                    if self.debug and attempt % 5 == 0:
                        logger.debug(f"Browser {index}: Waiting for Turnstile... attempt {attempt + 1}/15")

                # Additional wait for Turnstile to complete
                await asyncio.sleep(3)

                # Click login button
                logger.debug(f"Browser {index}: Clicking login button")
                login_button = page.locator('button[type="submit"], button:has-text("Giriş"), button:has-text("Login")').first
                ...
                if login_response and login_response.get('response', {}).get('success'):
                    token_data = login_response['response']['data']
                    logger.success(f"Browser {index}: Migros login successful in {elapsed_time}s")
                    ...
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "refExpiration": token_data.get('refExpiration'),
                    ...
                    if login_response:
                        error_msg = login_response.get('response', {}).get('errorMessage', 'Unknown error')

                    logger.error(f"Browser {index}: Migros login failed: {error_msg}")
                    return jsonify({
                        "success": False,
                        "elapsed_time": elapsed_time,
                        "error": error_msg
                ...
                elapsed_time = round(time.time() - start_time, 3)
                logger.error(f"Browser {index}: Migros login error: {str(e)}")
                return jsonify({
                    "success": False,
                    "elapsed_time": elapsed_time,
                    "error": str(e)
            ...
            logger.error(f"Migros login request error: {str(e)}")
